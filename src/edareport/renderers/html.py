...
<div class="container">
  ...
  <div class="section">
    ...
    <div class="plots-grid">
      {% for key, _ in scatter_plots.items() %}
...
<!-- GANTI seluruh blok <script> di _TEMPLATE dengan ini: -->
<script>
...
var _uniPlots = {{ uni_plot_json | tojson }};
Object.entries(_uniPlots).forEach(function([col, fig], i) {
  Plotly.newPlot("uni_" + (i + 1), _patch(fig).data, _patch(fig).layout, {responsive:true, displayModeBar:false});
});

{% if bi_heatmap_json %}
(function() {
  var fig = _patch({{ bi_heatmap_json | tojson }});
  Plotly.newPlot("bi_heatmap", fig.data, fig.layout, {responsive:true, displayModeBar:false});
})();
{% endif %}

var _scatterPlots = {{ scatter_plot_json | tojson }};
Object.entries(_scatterPlots).forEach(function([key, fig], i) {
  Plotly.newPlot("bi_" + (i + 1), _patch(fig).data, _patch(fig).layout, {responsive:true, displayModeBar:false});
});
</script>
</body>
</html>
...
class HtmlRenderer:
    ...
    def render(
        ...
        # Parse JSON string → dict supaya Jinja tidak double-encode
        uni_plots_parsed = {k: json.loads(v) for k, v in uni_plots.items()}
        scatter_plots_parsed = {k: json.loads(v) for k, v in scatter_plots.items()}
        bi_heatmap_parsed = (
            json.loads(bi_plots["correlation_heatmap"])
            if "correlation_heatmap" in bi_plots
            else None
        )
        ...
        return template.render(
            ...
            uni_plots=uni_plots,
            bi_plots=bi_plots,
            uni_plot_json=uni_plots_parsed,
            scatter_plot_json=scatter_plots_parsed,
            bi_heatmap_json=bi_heatmap_parsed,
        )
